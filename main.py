...
async def q8_guardrail(call: dict):
    ...
    if tool == "read_file":
        ...
        # Resolve path to check for escape
        ...
        if resolved == allowed_root or resolved.startswith(allowed_root + os.sep):
            try:
                ...
                return {"action": "allow", "reason": "Safe but missing file", "result": str(e)}
        else:
        ...
        # 2. Query param SSRF check (redirects to internal IPs)
        for vals in urllib.parse.parse_qs(u.query).values():
            for v in vals:
                v_lower = v.lower()
                if any(x in v_lower for x in ["169.254", "localhost", "127.0.0.1", "10.", "192.168"]):
